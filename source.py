...
async def on_message(message):
    if message.content.startswith('!help'):
    ...
    elif message.content.startswith('!langhelp'):
        channel = message.channel
        ...
        channel = message.channel
        new_str = message.content
        new_str = new_str.replace('!draw ','')
        new_ls = new_str.split(',')
        c_clc.generate_5_complex(int(new_ls[0]), int(new_ls[1]), int(new_ls[2]), int(new_ls[3]),int(new_ls[4]))
        await channel.send(file = discord.File('Science_Discord_Bot/plotimg/imgraphgui.png'))
